...
void NewStudent(Studentas &stud){							//Studento duomenu ivedimas
	bool knownGradesNum, randomGrades;			
	int gradeNum;
	...
	if(knownGradesNum){
		...
		if(randomGrades){							// random numeriu generavimas
		...
		stud.homeworkNum = rand() % 25 + 1;				// namu darbu range: 1-25
...
void processStud(Studentas stud, Studentas *&studentas, int &studNum){
...
int main(){
	...
	do{
		Studentas stud;
		NewStudent(stud);
		processStud(stud, studentas, studNum);
